...
let import () = result {
        ...
        arcticDatasetsWithPublications |> Seq.collect(fun (publication, datasets) ->
            ...
            if isOrphaned then
                ...
                |> List.tryFind(fun (k,title,year) ->
                    if year.IsSome then title.ToLower() = pubName.ToLower() && pubYear = year.Value
                    else title.ToLower() = pubName.ToLower())
                |> fun s -> 
                    match s with
                    | Some (a,b,c) -> 
                    ...
                    | None -> 
                        datasets |> Seq.map(fun (x,_) -> (x, (Graph.UniqueKey.FriendlyKey("sourcenode", sprintf "unknown_%s" pubName), pubName, None)))
        )

    let matchesString = System.Text.Json.JsonSerializer.Serialize(matches)
    ...
    let results = 
        matches |> Seq.collect(fun (datasetId, (key,name,_)) ->
            ...
            json.Data |> Array.map(fun data ->
                
                // 1. Get site details
                let lat, lon =
                    let m = System.Text.RegularExpressions.Regex.Match(data.Site.Geography, "\[(.*),(.*)\]")
                    float m.Groups.[1].Value, float m.Groups.[0].Value
                let location = Geography.Site((Geography.createLatitude lat |> Result.forceOk), (Geography.createLongitude lon |> Result.forceOk))
                let sampleOrigin = 
                    match data.Site.Collectionunit.Collunittype with
                    | "Core" ->
                        match data.Site.Collectionunit.Depositionalenvironment with
                        | "Mire" -> Population.Context.PeatCore
                        | _ -> Population.Context.LakeSediment
                ...
                let earliestExtentBP, latestExtentBP =
                    ...
                    | Some bpDate -> bpDate.Ageold, bpDate.Ageyoung
                    | None -> failwithf "Does not contain BP dates: %A" data.Site.Collectionunit.Dataset.Agerange

                // 3. Make proxied taxon nodes
                let existingTaxon = (Population.Taxonomy.Genus("Placeholder for NeotomaDB Import" |> Text.createShort |> Result.forceOk))
                ...
                let proxiedTaxa = 
                    ...
                    |> Seq.collect(fun s ->
                        s.Datum |> Seq.map(fun d ->
                            ...
                            | "pollen"
                            | "spore" -> Population.BioticProxies.Morphotype(Population.BioticProxies.Microfossil (Population.BioticProxies.Pollen, d.Variablename |> Text.createShort |> Result.forceOk))
                            | _ -> failwithf "Unknown proxy type: %s" d.Elementtype
                            )
                        ) |> Seq.distinct
                ...
                result {
                    ...
                    let! graph, sourceNode = 
                        if key.AsString.StartsWith "database_"
                        then
                            ...
                            let databaseCode = key.AsString.Split("_").[1]
                            let databaseNode = 
                                graph
                            ...
                            let databaseEntryNode : Sources.DatabaseDatasetNode = {
                                ...
                                UniqueIdentifierInDatabase = key.AsString.Split("_").[2] |> Text.createShort |> Result.forceOk
                    ...
                    let! (graphWithDatasetEntry,addedTimelineNode) = 
                        ...
                        |> fun g -> Storage.addNodes g [ 
                            (Node.PopulationNode <| PopulationNode.TaxonomyNode existingTaxon)
                            timelineNode
                            contextNode ]
